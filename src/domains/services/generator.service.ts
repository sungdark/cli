...
/**
 * Guards against pnpm browserslist shim issue (issue #1781).
 */
function guardBrowserslistForPnpm(outputDir: string): () => void {
  const key = 'BROWSERSLIST_ROOT_PATH';
  const original = process.env[key];
  process.env[key] = outputDir;
  return () => {
